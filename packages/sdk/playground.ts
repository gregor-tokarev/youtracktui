...
async function testStateChange() {
	try {
		...
		const issues = await youtrack.issues.search(
			"assignee: me #Unresolved Type: Task",
			{
				fields: [
					"id",
					"project(id,name)",
					"idReadable",
					"customFields(id,name,value(id,name),$type)",
				],
				top: 1,
			},
		);
		...
		const nextState = states.find(
			(s) => s.id !== currentStateId && !s.isResolved,
		);
		...
		const payloads = [
			...
			{
				name: "Format 2: { value: { id } }",
				payload: { value: { id: nextState.id } },
			},
			{
				name: "Format 3: { value: { id, $type } }",
				payload: { value: { id: nextState.id, $type: "StateBundleElement" } },
			},
			{
				name: "Format 4: { $type, value }",
				payload: {
					$type: "StateIssueCustomField",
					value: { id: nextState.id },
				},
			},
		];
		...
		for (const { name, payload } of payloads) {
			...
			try {
				...
				const refreshedStateField = refreshedIssue.customFields?.find(
					(cf) => cf.name === "State",
				);
